...
import { lineBreakConverter } from '../../../../src/ts/converter/format/line-break-converter';

describe('lineBreakConverter', () => {
  it('preProcess: 正常な文字数を正規化して成功を返す', async () => {
    const opts = { charsPerLine: ' 3 ' };

    const result = await lineBreakConverter.preProcess?.('abcdef', opts);
  ...
  it('convert: 指定文字数ごとに改行を入れる（マルチバイト文字対応）', async () => {
    const result = await lineBreakConverter.convert('あいうえお', { charsPerLine: '2' });
  ...
  it('convert: splitMode=charsの場合、従来通り文字数で改行する', async () => {
    const result = await lineBreakConverter.convert('abcde', { charsPerLine: '2', splitMode: 'chars' });
  ...
  it('convert: splitMode=utf8-bytesの場合、UTF-8バイト数で改行する', async () => {
    const result = await lineBreakConverter.convert('あaいb', { charsPerLine: '4', splitMode: 'utf8-bytes' });
  ...
  it('convert: splitMode=sjis-bytesの場合、ASCIIは1バイト、その他は2バイトで改行する', async () => {
    const result = await lineBreakConverter.convert('あaいb', { charsPerLine: '3', splitMode: 'sjis-bytes' });
  ...
  it('convert: splitMode=sjis-bytesの場合、半角カタカナは1バイトとして扱う', async () => {
    const result = await lineBreakConverter.convert('ｱｲｳあ', { charsPerLine: '3', splitMode: 'sjis-bytes' });
  ...
  it('convert: UTF-8/SJISバイト数モードでも文字の途中では分割しない', async () => {
    const utf8Result = await lineBreakConverter.convert('あい', { charsPerLine: '2', splitMode: 'utf8-bytes' });
    const sjisResult = await lineBreakConverter.convert('あい', { charsPerLine: '1', splitMode: 'sjis-bytes' });
  ...
  it('convert: 解釈不能な文字数は改行なしで連結する', async () => {
    const result = await lineBreakConverter.convert('ab\ncd', { charsPerLine: 'abc' });
  ...
  it('postProcess: 未実装（undefined）である', () => {
    expect(lineBreakConverter.postProcess).toBeUndefined();
  });
  ...
  describe('preProcess: 空白除去', () => {
    it('trimWhitespace=trueの場合、各行の行頭・行末の半角空白を除去する', async () => {
      ...
      const result = await lineBreakConverter.preProcess?.(' hello \n world ', opts);
  ...
  describe('preProcess: 改行除去', () => {
    ...
    it('removeLineBreaks=trueの場合、CRを除去する', async () => {
      ...
      const result = await lineBreakConverter.preProcess?.('hello\rworld', opts);
  ...
  describe('convert: 改行文字', () => {
    it('lineBreakCharが未指定の場合、\\nで区切る', async () => {
      const result = await lineBreakConverter.convert('abcdef', { charsPerLine: '3' });

      expect(result).toEqual({ success: true, output: 'abc\ndef' });
    });

    it('lineBreakChar=lfの場合、\\nで区切る', async () => {
      const result = await lineBreakConverter.convert('abcdef', { charsPerLine: '3', lineBreakChar: 'lf' });

      expect(result).toEqual({ success: true, output: 'abc\ndef' });
    });

    it('lineBreakChar=escaped-lfの場合、テキスト\\nで区切る', async () => {
      const result = await lineBreakConverter.convert('abcdef', { charsPerLine: '3', lineBreakChar: 'escaped-lf' });

      expect(result).toEqual({ success: true, output: 'abc\\ndef' });
    });

    it('lineBreakChar=escaped-crlfの場合、テキスト\\r\\nで区切る', async () => {
      const result = await lineBreakConverter.convert('abcdef', { charsPerLine: '3', lineBreakChar: 'escaped-crlf' });

      expect(result).toEqual({ success: true, output: 'abc\\r\\ndef' });
    });

    it('lineBreakChar=brの場合、<br>で区切る', async () => {
      const result = await lineBreakConverter.convert('abcdef', { charsPerLine: '3', lineBreakChar: 'br' });

      expect(result).toEqual({ success: true, output: 'abc<br>def' });
    });

    it('lineBreakChar=noneの場合、区切り文字なしで連結する', async () => {
      const result = await lineBreakConverter.convert('abcdef', { charsPerLine: '3', lineBreakChar: 'none' });
  ...
  describe('convert: 分割モード', () => {
    ...
    it('splitModeが不正値でも変換処理を継続する', async () => {
      const result = await lineBreakConverter.convert('abcdef', { charsPerLine: '3', splitMode: 'invalid' });

      expect(result).toEqual({ success: true, output: 'abc\ndef' });
    });

    it('charsPerLineが0以下の場合、改行なしで連結する', async () => {
      const zeroResult = await lineBreakConverter.convert('ab\ncd', { charsPerLine: '0', lineBreakChar: 'lf' });
      const negativeResult = await lineBreakConverter.convert('ab\ncd', { charsPerLine: '-1', lineBreakChar: 'lf' });
  ...
  describe('convert: 禁則処理', () => {
    it('kinsoku=trueの場合、行頭禁則文字を前の行に移動する', async () => {
      ...
      const result = await lineBreakConverter.convert('abcd。', { charsPerLine: '2', kinsoku: true });
    ...
    it('kinsoku=trueの場合、行末禁則文字を次の行に移動する', async () => {
      ...
      const result = await lineBreakConverter.convert('a（bcde', { charsPerLine: '2', kinsoku: true });
    ...
    it('kinsoku=trueの場合、行頭禁則文字が連続しても正しく処理する', async () => {
      ...
      const result = await lineBreakConverter.convert('ab、。cd', { charsPerLine: '2', kinsoku: true });
    ...
    it('kinsoku=falseまたは未指定の場合、禁則処理を行わない', async () => {
      ...
      const result = await lineBreakConverter.convert('abcd。', { charsPerLine: '2' });
