...
import { randomTextGenerator } from '../../../../src/ts/converter/utility/random-text-generator';

describe('randomTextGenerator', () => {
  describe('repeat モード', () => {
    it('入力テキストを指定文字数分繰り返す', async () => {
      const result = await randomTextGenerator.convert('abc', { mode: 'repeat', length: '7', lines: '1' });

      expect(result).toEqual({ success: true, output: 'abcabca' });
    });

    it('入力テキストの改行文字を除いてリピートする', async () => {
      const result = await randomTextGenerator.convert('AB\nC', { mode: 'repeat', length: '7', lines: '1' });

      expect(result).toEqual({ success: true, output: 'ABCABCA' });
    });

    it('行数分の行を出力する', async () => {
      const result = await randomTextGenerator.convert('ab', { mode: 'repeat', length: '3', lines: '2' });

      expect(result).toEqual({ success: true, output: 'aba\naba' });
    });

    it('入力テキストが空の場合は空文字列を返す', async () => {
      const result = await randomTextGenerator.convert('', { mode: 'repeat', length: '5', lines: '1' });
  ...
  describe('shuffle モード', () => {
    it('指定文字数を出力する', async () => {
      const result = await randomTextGenerator.convert('hello', { mode: 'shuffle', length: '8', lines: '1' });
    ...
    it('出力文字が入力テキストの文字のみからなる', async () => {
      const result = await randomTextGenerator.convert('abc', { mode: 'shuffle', length: '6', lines: '1' });
  ...
  describe('guid モード', () => {
    it('UUID v4形式の文字列を出力する', async () => {
      const result = await randomTextGenerator.convert('', { mode: 'guid', lines: '1' });
    ...
    it('指定行数分のGUIDを出力する', async () => {
      const result = await randomTextGenerator.convert('', { mode: 'guid', lines: '3' });
  ...
  describe('password モード', () => {
    it('数字のみの場合、数字のみのパスワードを生成する', async () => {
      const result = await randomTextGenerator.convert('', {
