  ...
  description: () => `
    ...
    <div class="converter-options">
      <div>
        ...
        <select id="opt-mode">
          <option value="repeat" selected>入力テキストのリピート</option>
          <option value="shuffle">入力テキストをシャッフル</option>
          <option value="guid">GUID</option>
          <option value="password">パスワード</option>
        </select>
      </div>
      <div>
        <label for="opt-length">文字数（GUIDモードでは無視）</label>
        <input id="opt-length" type="number" value="10" min="1">
      </div>
      <div>
        <label for="opt-lines">行数</label>
        <input id="opt-lines" type="number" value="1" min="1">
      </div>
      <div>
        <span>文字種（パスワードモード）</span><br>
        ...
        <label><input id="opt-useUppercase" type="checkbox" checked> 大文字アルファベット</label>
        <label><input id="opt-useSymbols" type="checkbox"> 記号</label>
      </div>
    </div>
  `,
  async convert(text, opts) {
    const mode = (opts['mode'] ?? 'repeat') as GenerateMode;
    const length = Number.parseInt(String(opts['length'] ?? '10'), 10);
    const lines = Number.parseInt(String(opts['lines'] ?? '1'), 10);
    ...
    if (opts['useSymbols'] === true) pools.push(SYMBOLS);
